 ...
 *
 * _Intermediate operators_ on the flow such as [map], [filter], [take], [zip], and etc are functions that are
 ...
 *
 * _Terminal operators_ on the flow are suspending functions such as [collect], [single], [reduce], [toList], and etc.
 * They are are applied to the upstream flow and trigger execution of all operations.
 ...
 * ```
 *
 * Failure to adhere to the exception transparency requirement would strange behaviours that would make it hard
 * to reason about the code as exception in the `collect { ... }` would be somehow "caught" by upstream flows, limiting
 * ability of local reasoning about the code.
 *
